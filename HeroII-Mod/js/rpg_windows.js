...
Window_Help.prototype.initialize = function(numLines) {
    var width = Graphics.boxWidth-150;//宽度调整
    var height = this.fittingHeight(numLines || 1);//帮助高度
    Window_Base.prototype.initialize.call(this, 150, 0, width, height);
    this._text = '';
};
...
Window_MenuCommand.prototype.addMainCommands = function() {
    ...
    if (this.needsCommand('cancel')) {
        this.addCommand(TextManager.cancel, 'cancel', enabled);
    }//新增加一个取消返回按钮
...
Window_ItemList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x, y-2, width, height);
    this._category = 'none';
    this._data = [];
};
...
Window_SkillList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x+70, y+45, width-140, height-100);//菜单
    this._actor = null;
    this._stypeId = 0;
    this._data = [];
};
...
Window_EquipStatus.prototype.refresh = function() {
    this.contents.clear();
    if (this._actor) {
        this.drawActorName(this._actor, this.textPadding()+25, 0);//人物姓名
        for (var i = 0; i < 6; i++) {
...
Window_EquipStatus.prototype.drawItem = function(x, y, paramId) {
    this.drawParamName(x + this.textPadding()+25, y, paramId);//属性
    if (this._actor) {
        this.drawCurrentParam(x + 110, y, paramId);
    }
    this.drawRightArrow(x + 170, y);
    if (this._tempActor) {
        this.drawNewParam(x + 200, y, paramId);
    }
};
...
Window_EquipSlot.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x-20, y-20, width+30, height+10);
    this._actor = null;
    this.refresh();
};
...
Window_EquipSlot.prototype.drawItem = function(index) {
    if (this._actor) {
        ...
        this.drawText(this.slotName(index), rect.x+4, rect.y, 138, this.lineHeight());
        this.drawItemName(this._actor.equips()[index], rect.x-3, rect.y+2);//rect.x+138
        this.changePaintOpacity(true);
    }
};
...
Window_Options.prototype.updatePlacement = function() {
    ...
    this.y = (Graphics.boxHeight - this.height) / 2 - 80;//设置
};
...
Window_NameEdit.prototype.charWidth = function() {
    ...
    return this.textWidth(text)+15;//名字字间距
};
