...
Scene_Gobang.prototype.createMessageWindow = function() {
    ...
    this._messageWindow = new Window_Base(0, Graphics.boxHeight * 0.25, width, height);
	...
	this._messageWindow2.drawText('你', 0, 0, width - padding*2, 'center');
	this._messageWindow2.drawText('下', 0, height/3, width - padding*2, 'center');
	var content = this._player ? '白':'黑';
	this._messageWindow2.drawText(content, 0, height*2/3, width - padding*2, 'center');
	this.updateContent('步数', this._step + '');
};
	...
	if(this._loadingTime > 0){
		...
		if(this.updateChess()){
			if(this._result == true){
				...
				content = '赢了!';
				this.updateContent(content, '恭喜');
			}
			else{
				$gameVariables.setValue(KJ.Gobang.Param.VariableId, 2);
				content = '输了!';
				this.updateContent(content, '遗憾');
			}
			this._lastcurrent = -1;
...
Scene_Gobang.prototype.updateContent = function(content1, content2){
	var padding = this._messageWindow.padding;
	var height = Graphics.boxHeight * 0.5;
	var width = (Graphics.boxWidth - Graphics.boxHeight * 0.9) / 2;
	this._messageWindow.contents.clearRect(0, 0, width, height);
	this._messageWindow.drawText(content1, 0, 0, width - padding*2, 'center');
	this._messageWindow.drawText(content2, 0, height/2, width - padding*2, 'center');
};
