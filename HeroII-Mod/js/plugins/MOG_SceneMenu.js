...
ImageManager.loadMenusFaces1 = function(filename) {
    return this.loadBitmap('img/menus/faces/', filename, 0, true);
};	
...
Scene_Menu.prototype.loadBitmapsMain = function() {
    ...
    for (var i = 0; i < this._comList.length; i++) {
		 //this._comBitmaps[i] = ImageManager.loadMenusMainCommands(this._comList[i].name);//主菜单
		this._comBitmaps[i] = ImageManager.loadMenusMainCommands("xiao_" + i);
    };
...
Scene_Menu.prototype.createCommands = function() {
	 ...
	 for (var i = 0; i < this._comList.length; i++) {
		 ...
		 this._compos[i] = [Moghunter.scMenu_ComX + (78 * i),Moghunter.scMenu_ComY + (48 * h)];
		 this._comzoom[i] = 0;
...
MBustMenu.prototype.createCharaters = function() {
	this._char = new Sprite(ImageManager.loadPicture("Actor_" + this._actor._actorId));
	 //this._char = new Sprite(ImageManager.loadMenusFaces3("Actor_" + this._actor._actorId));
     //this._char = new Sprite(ImageManager.loadPicture("Actor_" + $gameVariables.value(18)));
...
MCharStatus.prototype.createStates = function() {
	...
	this._state_icon.y = this._layout.y + Moghunter.scMenu_statesY - 110;
