...
//=============================================================================
 /*:
 * @plugindesc v1.24 团队核心插件，改变游戏团队界面
...
Window_ItemList.prototype.initialize = function(x, y, width, height) {
    if (SceneManager._scene instanceof Scene_Item) {
      width = Graphics.boxWidth / 1.39;
      y=284;
      height=260;
    }
    Yanfly.Item.Window_ItemList_initialize.call(this, x, y, width, height);//列表框
};

Yanfly.Item.Window_ItemList_maxCols = Window_ItemList.prototype.maxCols;
Window_ItemList.prototype.maxCols = function() {
    if (SceneManager._scene instanceof Scene_Item) return 2;//列表列数
...
Window_ItemStatus.prototype.initialize = function(x, y, width, height) {
    Window_Base.prototype.initialize.call(this, x-5, y, width, height);//状态栏
    this._item = null;
    this.deactivate();
    this.refresh();
};
...
Window_ItemStatus.prototype.drawEquipInfo = function(item) {
    ...
    for (var i = 0; i < 8; ++i) {
      ...
      if (item.params[i] >= 0) text = '﹢' + text;//加号
      if (text === '+0') this.changePaintOpacity(false);
      this.drawText(text, dx, rect.y, dw-10, 'right');
      this.changePaintOpacity(true);
    }
};
...
Window_ItemStatus.prototype.drawItemData = function(i, dx, dy, dw) {
    ...
    if (icons.length > 0) {
      ...
      text = pre + value + text;
      this.drawText(text, dx-10, dy, dw, 'right');
      this.changePaintOpacity(true);
    }
};
...
Scene_Item.prototype.createStatusWindow = function() {
    ...
    var ww = Graphics.boxWidth - wx-20;//状态栏宽度
...
Scene_Item.prototype.createInfoWindow = function() {
    ...
    var ww = Graphics.boxWidth - wx-5;//右下方信息框
