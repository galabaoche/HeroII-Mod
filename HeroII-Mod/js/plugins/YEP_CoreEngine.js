...
//=============================================================================
/*:
 * @plugindesc v1.18 主体核心插件，是所有插件的前提条件。
...
Window_Base.prototype.drawActorSimpleStatus = function(actor, x, y, width) {
    ...
    this.drawActorName(actor, x+18, y);
    //this.drawActorLevel(actor, x, y + lineHeight * 1);
    this.drawActorIcons(actor, x, y + lineHeight * 1.1);
    //this.drawActorIcons(actor, x, y + lineHeight * 2);
    this.drawActorClass(actor, x, y + lineHeight * 2);//门派
    this.changeTextColor(this.hpGaugeColor2());
    this.drawText("食物：" + $gameVariables.value(1)+"/"+$gameVariables.value(2), x2+80, y , width2);
    this.changeTextColor(this.crisisColor());
    this.drawText("饮水：" + $gameVariables.value(3)+"/"+$gameVariables.value(4), x2+330, y , width2);
    this.changeTextColor(this.normalColor());
    this.drawText("难度：" + $gameActors._data[2]._nickname, x2+80, y + lineHeight* 2, width2);
    this.drawText("职位：" + $gameActors._data[3]._nickname, x2+330, y + lineHeight* 2, width2);
    //this.drawActorHp(actor, x2, y + lineHeight * 1, width2);
    //this.drawActorMp(actor, x2, y + lineHeight * 2, width2);
    /*if (eval(Yanfly.Param.MenuTpGauge)) {
        this.drawActorTp(actor, x2 * 1.4, y + lineHeight * 3, width2 / 2);
    }*/
};
...
Window_SkillStatus.prototype.refresh = function() {
    ...
    if (this._actor) {
        ...
        this.drawActorSimpleStatus(this._actor, xpad, y, width);//状态框
    }
};
...
Window_SkillType.prototype.makeCommandList = function() {
    if (this._actor) {
        ...
        skillTypes.forEach(function(stypeId) {
            ...
            //this.addCommand(TextManager.cancel, 'cancel',true, stypeId);//增加一个返回按钮
        }, this);
