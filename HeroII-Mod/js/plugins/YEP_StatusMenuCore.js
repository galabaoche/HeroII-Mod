...
Window_StatusCommand.prototype.windowWidth = function() {
    return Yanfly.Param.StatusCmdWidth-150;//xiaorui
};
...
Window_StatusInfo.prototype.drawGeneral = function() {
		...
		this.drawText("年龄："+$gameVariables.value(15)+" 岁", dx-40, dy, dw, 'center');
		dx += this.contents.width / 2;
        this.drawText($gameActors._data[1]._nickname, dx-70, dy, dw, 'center');
		//this.drawText(Yanfly.Param.StatusExpText, dx, dy, dw, 'center');//名字
...
Window_StatusInfo.prototype.drawGeneralParam = function() {
    ...
    rect.width = (this.contents.width - this.standardPadding()) / 3;
    ...
    this.changeTextColor(this.systemColor());
		this.drawText("江湖经验", dx, rect.y, dw, 'left');

		this.changeTextColor(this.normalColor());
		text = Yanfly.Util.toGroup(this._actor.level);
		//this.drawText(text, dx, rect.y, dw, 'right');//级别
        this.drawText($gameVariables.value(6), dx, rect.y, dw, 'right');//级别

 rect.y = this.lineHeight() * 3;//行距
 this.drawDarkRect(rect.x, rect.y, rect.width, rect.height);//画框
 this.changeTextColor(this.systemColor());//颜色
 this.drawText("武艺潜能", dx, rect.y, dw, 'left');
 this.changeTextColor(this.normalColor());
 text = Yanfly.Util.toGroup(this._actor.level);
 //this.drawText(text, dx, rect.y, dw, 'right');//名字
 this.drawText($gameVariables.value(5), dx, rect.y, dw, 'right');//数值
...
/* rect.y += this.lineHeight();
 ...
 this.drawDarkRect(rect.x, rect.y, rect.width, rect.height);
 this.changeTextColor(this.systemColor());
 this.drawText("fff", dx, rect.y, dw, 'left');
 this.changeTextColor(this.normalColor());
 text = "44";
 this.drawText(text, dx, rect.y, dw, 'right');
...
Window_StatusInfo.prototype.drawGeneralExp = function(dx, dy, dw, dh) {
    ...
    dw = (this.contents.width - this.textPadding()) / 2-30;
    ...
    dy += this.lineHeight();
    this.drawText("善恶：" + $gameVariables.value(10), dx, dy, dw, 'left');
    this.drawText("已杀：" + $gameVariables.value(9) + " 人", dx, dy, dw, 'right');
    dy += this.lineHeight();
    this.drawText("实战：" + $gameVariables.value(11), dx, dy, dw, 'left');
    this.drawText("已完成：" + $gameVariables.value(12) + " 个任务", dx, dy, dw, 'right');
 dy += this.lineHeight();
 this.drawText("修为：" + $gameSystem.playtime(), dx, dy, dw, 'left');
 this.drawText("武林排名：" + "第 " + $gameVariables.value(13) + " 名", dx, dy, dw, 'right');
 dy += this.lineHeight();
 this.drawText("阅历：" +$gameParty.steps(), dx, dy, dw, 'left');
 this.drawText("江湖人缘：" + $gameVariables.value(14), dx, dy, dw, 'right');
...
Scene_Status.prototype.refreshActor = function() {
        ...
        this._helpWindow.visible = false//隐藏帮助窗口
};
...
Scene_Status.prototype.createCommandWindow = function() {
        ...
        //this._commandWindow.width =960;
        this._commandWindow.width =this._helpWindow.width;//菜单宽度+150
...
Scene_Status.prototype.createStatusWindow = function() {
    ...
    var wh = this._commandWindow.height+105;
...
Scene_Status.prototype.createInfoWindow = function() {
           //var wy = this._commandWindow.height;
         //var wy = this._helpWindow.height + this._commandWindow.height+105;
        var wy =this._commandWindow.height+185;
