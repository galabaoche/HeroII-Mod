...
ImageManager.isReady = function() {
    for (var key in this.cache._inner) {
        ...
        if (bitmap.isError()) {
            throw new Error("无法加载:" + bitmap.url);
        }
        if (!bitmap.isReady()) {
            return false;
        }
...
AudioManager.checkWebAudioError = function(webAudio) {
    if (webAudio && webAudio.isError()) {
        throw new Error("无法加载:" + webAudio.url);
    }
};
...
SceneManager.checkWebGL = function() {
    if (!Graphics.hasWebGL()) {
        throw new Error("您的游戏设备不支持显卡技术");//WebGL
    }
};

SceneManager.checkFileAccess = function() {
    if (!Utils.canReadGameFiles()) {
        throw new Error("您的游戏设备不允许读取本地文件。");
    }
};
...
SceneManager.initAudio = function() {
    ...
    if (!WebAudio.initialize(noAudio) && !noAudio) {
        throw new Error("您的游戏设备不支持音频解码");//Web Audio API
    }
};
...
SceneManager.onError = function(e) {
    ...
    try {
        this.stop();
        Graphics.printError('出错啦！', e.message);//Error
